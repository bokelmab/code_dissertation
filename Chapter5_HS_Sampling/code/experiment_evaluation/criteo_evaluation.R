...
for(i_samp_met in 1:length(samp_methods)){
  ...
  path_results <- paste0('results/criteo/', samp_param_method)
  ...
  for(i_fold in 1:13){
    ...
    phi_hat <- c(phi_hat, prob_W*predict(rf0, dt_test[(1:1000000)+(i_fold-1)*1000000,])$predictions+(1-prob_W)*predict(rf1, dt_test[(1:1000000)+(i_fold-1)*1000000,])$predictions) ## add phihat predictions for covariate adjustment to experimental data
    print(i_fold)
  }
  ...
  phi_hat <- c(phi_hat, prob_W*predict(rf0, dt_test[13000001:nrow(dt_test),])$predictions+(1-prob_W)*predict(rf1, dt_test[13000001:nrow(dt_test),])$predictions) ## add phihat predictions for covariate adjustment to experimental data
  ...
  for(i_boot in 1:1000){
    ...
    ## save results
    saveRDS(est_ate_ca, paste0('results/', 'criteo/', samp_param_method, '/est_ate_ca.RDS'))
    saveRDS(est_ate_het, paste0('results/', 'criteo/', samp_param_method, '/est_ate_het.RDS'))
    saveRDS(est_ate_hsca, paste0('results/', 'criteo/', samp_param_method, '/est_ate_hsca.RDS'))
    saveRDS(est_ate_unif, paste0('results/', 'criteo/', samp_param_method, '/est_ate_unif.RDS'))
    
    saveRDS(var_ate_ca, paste0('results/', 'criteo/', samp_param_method, '/var_ate_ca.RDS'))
    saveRDS(var_ate_het, paste0('results/', 'criteo/', samp_param_method, '/var_ate_het.RDS'))
    saveRDS(var_ate_hsca, paste0('results/', 'criteo/', samp_param_method, '/var_ate_hsca.RDS'))
    saveRDS(var_ate_unif, paste0('results/', 'criteo/', samp_param_method, '/var_ate_unif.RDS'))
    
    saveRDS(qini_table_ca, paste0('results/', 'criteo/', samp_param_method, '/qini_table_ca.RDS'))
    saveRDS(qini_table_het, paste0('results/', 'criteo/', samp_param_method, '/qini_table_het.RDS'))
    saveRDS(qini_table_hsca, paste0('results/', 'criteo/', samp_param_method, '/qini_table_hsca.RDS'))
    saveRDS(qini_table_unif, paste0('results/', 'criteo/', samp_param_method, '/qini_table_unif.RDS'))
    
    saveRDS(var_qini_table_ca, paste0('results/', 'criteo/', samp_param_method, '/var_qini_table_ca.RDS'))
    saveRDS(var_qini_table_het, paste0('results/', 'criteo/', samp_param_method, '/var_qini_table_het.RDS'))
    saveRDS(var_qini_table_hsca, paste0('results/', 'criteo/', samp_param_method, '/var_qini_table_hsca.RDS'))
    saveRDS(var_qini_table_unif, paste0('results/', 'criteo/', samp_param_method, '/var_qini_table_unif.RDS'))
