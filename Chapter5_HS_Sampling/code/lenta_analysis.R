...
dt_exp$pred_prob <- predict(rf_base, dt_exp)$predictions

opt_split_data1 <- find_split(dt_exp)
plot((0:100)/100,1-opt_split_data1$var_rel_opt, type = 'l', ylim = c(0,1-min(opt_split_data1$var_rel_opt)), ylab = '% variance reduction', 
     xlab = TeX('$p_L$'), lwd = 3, main = 'Lenta')
lines((0:100)/100, 1-opt_split_data1$var_rel_prop, col = 'red', lwd = 3)

pL <- 5/4*(which.min(opt_split_data1$var_rel_opt)-1)/100-1/4

pred_var1 <- mean(dt_exp[pred_prob <= quantile(pred_prob, pL),]$pred_prob)*(1-mean(dt_exp[pred_prob <= quantile(pred_prob, pL),]$pred_prob))
pred_var2 <- mean(dt_exp[pred_prob > quantile(pred_prob, pL),]$pred_prob)*(1-mean(dt_exp[pred_prob > quantile(pred_prob, pL),]$pred_prob))
...
dt_exp$var_group <- ifelse(dt_exp$pred_prob > quantile(dt_exp$pred_prob,pL), 2, 1)

## training of uplift model
idx_train <- sample(1:nrow(dt_exp), 100000, replace = F)
dt_train <- dt_exp[idx_train,]
dt_test <- dt_exp[-idx_train,]
rf0 <- ranger(Y ~ ., data = dt_train[W==0,-c('W','var_group', 'pred_prob'), with = F], max.depth = 5)
rf1 <- ranger(Y ~ ., data = dt_train[W==1,-c('W','var_group', 'pred_prob'), with = F], max.depth = 5)
...
for(i_boot in 1:1000){
  ...
  ## save results
  # saveRDS(est_ate_ca, paste0('results/', 'lenta', '/est_ate_ca.RDS'))
  # saveRDS(est_ate_het, paste0('results/', 'lenta', '/est_ate_het.RDS'))
  # saveRDS(est_ate_hsca, paste0('results/', 'lenta', '/est_ate_hsca.RDS'))
  # saveRDS(est_ate_unif, paste0('results/', 'lenta', '/est_ate_unif.RDS'))
  # 
  # saveRDS(var_ate_ca, paste0('results/', 'lenta', '/var_ate_ca.RDS'))
  # saveRDS(var_ate_het, paste0('results/', 'lenta', '/var_ate_het.RDS'))
  # saveRDS(var_ate_hsca, paste0('results/', 'lenta', '/var_ate_hsca.RDS'))
  # saveRDS(var_ate_unif, paste0('results/', 'lenta', '/var_ate_unif.RDS'))
  # 
  # saveRDS(qini_table_ca, paste0('results/', 'lenta', '/qini_table_ca.RDS'))
  # saveRDS(qini_table_het, paste0('results/', 'lenta', '/qini_table_het.RDS'))
  # saveRDS(qini_table_hsca, paste0('results/', 'lenta', '/qini_table_hsca.RDS'))
  # saveRDS(qini_table_unif, paste0('results/', 'lenta', '/qini_table_unif.RDS'))
  # 
  # saveRDS(var_qini_table_ca, paste0('results/', 'lenta', '/var_qini_table_ca.RDS'))
  # saveRDS(var_qini_table_het, paste0('results/', 'lenta', '/var_qini_table_het.RDS'))
  # saveRDS(var_qini_table_hsca, paste0('results/', 'lenta', '/var_qini_table_hsca.RDS'))
  # saveRDS(var_qini_table_unif, paste0('results/', 'lenta', '/var_qini_table_unif.RDS'))
...
# fwrite(dt_exp, paste0('results/', 'lenta', '/dt_exp.csv'))
# fwrite(dt_base, paste0('results/', 'lenta', '/dt_base.csv'))
...
for(i_sim in 1:1000){
  ...
  ## T-learner
  rf0_unif <- ranger(Y ~ ., data = train_unif[W==0,-c('W','var_group', 'pred_prob'), with = F], max.depth = 5)
  rf1_unif <- ranger(Y ~ ., data = train_unif[W==1,-c('W','var_group', 'pred_prob'), with = F], max.depth = 5)
  ...
  ## S-Learner
  rfs_unif <- ranger(Y ~ ., data = train_unif[, -c('var_group', 'pred_prob'), with = F], max.depth = 5)
  ...
  train_unif_x1$Y <- train_unif_x1$Y - predict(rf0_unif, train_unif_x1[,-c('W','var_group', 'pred_prob'),with = F])$predictions
  train_unif_x0$Y <- predict(rf1_unif, train_unif_x0[,-c('W','var_group', 'pred_prob'),with = F])$predictions - train_unif_x0$Y
  rfX0_unif <- ranger(Y ~ ., data = train_unif_x0[,-c('W','var_group', 'pred_prob'), with = F], max.depth = 5)
  rfX1_unif <- ranger(Y ~ ., data = train_unif_x1[,-c('W','var_group', 'pred_prob'), with = F], max.depth = 5)
  predx_unif <- (1-prob_W)*predict(rfX0_unif, data = dt_test)$predictions+prob_W*predict(rfX1_unif, data = dt_test)$predictions
  auqcx_unif_rf <- c(auqcx_unif_rf, calc_auq(predx_unif, dt_test))

  ## Causal random forest
  cf_unif <- causal_forest(X = train_unif[,-c('W', 'Y','var_group', 'pred_prob'), with = F], Y = train_unif$Y, W = train_unif$W, W.hat = prob_W, num.trees = 500)
  predcf_unif <- predict(cf_unif, dt_test[,-c('W', 'Y','var_group', 'pred_prob'), with = F])$predictions
  ...
  cbart <- bartc(response = train_unif$Y, treatment = train_unif$W, 
                 confounders = train_unif[,-c('W', 'Y','var_group', 'pred_prob'), with = F], 
  ...
  ## T-learner
  rf0_het <- ranger(Y ~ ., data = train_het[W==0,-c('W','var_group', 'pred_prob'), with = F], max.depth = 5)
  rf1_het <- ranger(Y ~ ., data = train_het[W==1,-c('W','var_group', 'pred_prob'), with = F], max.depth = 5)
  ...
  ## S-Learner
  rfs_het <- ranger(Y ~ ., data = train_het[,-c('var_group', 'pred_prob'), with = F], max.depth = 5)
  ...
  train_het_x1$Y <- train_het_x1$Y - predict(rf0_het, train_het_x1[,-c('W','var_group', 'pred_prob'),with = F])$predictions
  train_het_x0$Y <- predict(rf1_het, train_het_x0[,-c('W','var_group', 'pred_prob'),with = F])$predictions - train_het_x0$Y
  rfX0_het <- ranger(Y ~ ., data = train_het_x0[,-c('W','var_group', 'pred_prob'), with = F], max.depth = 5)
  rfX1_het <- ranger(Y ~ ., data = train_het_x1[,-c('W','var_group', 'pred_prob'), with = F], max.depth = 5)
  predx_het <- (1-prob_W)*predict(rfX0_het, data = dt_test)$predictions+prob_W*predict(rfX1_het, data = dt_test)$predictions
  auqcx_het_rf <- c(auqcx_het_rf, calc_auq(predx_het, dt_test))

  ## Causal random forest
  cf_het <- causal_forest(X = train_het[,-c('W', 'Y','var_group', 'pred_prob'), with = F], Y = train_het$Y, W = train_het$W, W.hat = prob_W, num.trees = 500)
  predcf_het <- predict(cf_het, dt_test[,-c('W', 'Y','var_group', 'pred_prob'), with = F])$predictions
  ...
  cbart_het <- bartc(response = train_het$Y, treatment = train_het$W, 
                     confounders = train_het[,-c('W', 'Y','var_group', 'pred_prob'), with = F], 
  ...
  ## save results
  # saveRDS(auqct_het_rf, paste0('results/', 'lenta', '/auqct_het_rf.RDS'))
  # saveRDS(auqcs_het_rf, paste0('results/', 'lenta', '/auqcs_het_rf.RDS'))
  # saveRDS(auqcx_het_rf, paste0('results/', 'lenta', '/auqcx_het_rf.RDS'))
  # saveRDS(auqccf_het, paste0('results/', 'lenta', '/auqccf_het.RDS'))
  # saveRDS(auqccb_het, paste0('results/', 'lenta', '/auqccb_het.RDS'))
  
  # saveRDS(auqct_unif_rf, paste0('results/', 'lenta', '/auqct_unif_rf.RDS'))
  # saveRDS(auqcs_unif_rf, paste0('results/', 'lenta', '/auqcs_unif_rf.RDS'))
  # saveRDS(auqcx_unif_rf, paste0('results/', 'lenta', '/auqcx_unif_rf.RDS'))
  # saveRDS(auqccf_unif, paste0('results/', 'lenta', '/auqccf_unif.RDS'))
